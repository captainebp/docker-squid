...
function squid_config() {
    ...
    echo "pid_filename /tmp/squid.pid" >> /etc/squid/squid.conf
    echo "cache_effective_group proxy" >> /etc/squid/squid.conf
	...
	if ${HTTPS_PORT_ENABLE}
    then
        echo "LDAP Enable"
        echo -e "https_port 8443 tls-cert=${SERVER_TLS_CERT_PATH}" >> /etc/squid/squid.conf
    fi
}
